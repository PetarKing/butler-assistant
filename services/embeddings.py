...
class EmbeddingService:
    ...
    def load(self) -> None:
        ...
        if os.path.exists(self.db_path):
            ...
            try:
                ...
                print(f"⚠️ Error loading RAG index. Run 'build_index.py' to rebuild. Error: {e}")
        else:
            print(f"⚠️ Database path '{self.db_path}' not found. Run 'build_index.py' to create it.")
        ...
        for doc in docs:
            try:
                ...
                if note_realpath.startswith(prefix_to_remove):
                    doc.metadata['relative_path'] = note_realpath[len(prefix_to_remove):]
                else:
                    raise ValueError(f"Note path '{note_realpath}' not inside vault root '{vault_realpath}'.")
                ...
                raise RuntimeError(f"Failed to process document '{doc.metadata.get('path')}': {e}")
    ...
    def search_with_tag_filter(self, query: str, tag: str) -> list[dict]:
        ...
        results = self.vector_store.similarity_search(
            query, 
            k=SEMANTIC_SEARCH_K_VALUE, 
            filter={"tags": {"$like": f"%{tag}%"}}
        )
        return self._format_results(results)
