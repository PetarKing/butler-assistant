...
from config.settings import (CHUNK, DEBUG_AUDIO, ENERGY_THRESHOLD,
                             MAX_RECORD_SEC, RATE, TRAILING_SILENCE_SEC,
                             VAD_AGGRESSIVENESS, VAD_FRAME_MS, WAITING_VOLUME,
                             WAITING_WAV)
...
async def record(
    max_sec: int = MAX_RECORD_SEC, silence: int = ENERGY_THRESHOLD
) -> str | None:
    """
        ...
        with sd.InputStream(
            ...
            for _ in range(max_chunks):
                ...
                while len(buffer) >= FRAME_BYTES:
                    ...
                    if (
                        heard_voice
                        and (frame_index - last_voiced) >= silence_limit_frames
                    ):
...
async def play_waiting_music(stop_event: asyncio.Event) -> None:
    ...
    try:
        ...
        sample_rate = 44100
        t = np.linspace(
            0, 0.25, int(sample_rate * 0.25), endpoint=False, dtype=np.float32
        )
    ...
    try:
        with sd.RawOutputStream(
            samplerate=sample_rate, channels=data.shape[1], dtype="float32", blocksize=0
        ) as output_stream:
            ...
            while not stop_event.is_set():
                ...
                if end_pos >= len(data):
                    chunk = np.vstack(
                        [data[position:], data[: end_pos - len(data)]])
