...
from config.settings import (
    IDLE_EXIT_SEC, STT_MODEL,
    INCLUDE_OBSIDIAN_TOOLS, ENABLE_SEMANTIC_SEARCH, USE_CORE_MEMORY
)
from config.personality import ASSISTANT_NAME
from agents.butler_agent import ButlerAgent
from services.audio_service import record, play_waiting_music
...
def initialize_app_tools():
    ...
    for schema in final_schemas:
        ...
        if 'function' not in schema or 'name' not in schema.get('function', {}):
             print(f"⚠️ Warning: Discarding invalid schema object after normalization: {schema}")
             continue
    ...
    sorted_tools = sorted(final_schemas, key=lambda x: x.get('function', {}).get('name', ''))
    ...
    final_unique_implementations = {name: final_implementations[name] for name in seen_tool_names if name in final_implementations}
...
async def voice_chat(tool_implementations, tool_schemas):
    ...
    print(f"👋 Hello! I'm your butler {ASSISTANT_NAME}. How can I assist you today?")
